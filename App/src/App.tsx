...
// Global Google Analytics event tracking utility
export const trackGAEvent = (eventName: string, eventParams?: Record<string, any>) => {
  if (typeof window !== 'undefined' && window.gtag) {
    window.gtag('event', eventName, eventParams);
  } else {
    console.log('[Analytics Dry Run]', eventName, eventParams);
  }
};
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (!measurementId) {
      console.log('Google Analytics: VITE_GA_MEASUREMENT_ID is not configured in environment variables.');
      return;
    }
    ...
    if (!script) {
      ...
      console.log('Google Analytics successfully initialized with ID:', measurementId);
    }
  }, []);
  ...
  // Track search queries in Google Analytics with a 1.5s debounce to avoid spamming while typing
  useEffect(() => {
    if (!searchQuery) return;
    const timer = setTimeout(() => {
      trackGAEvent('search', {
        search_term: searchQuery,
        room_type: roomTypeTab
      });
    }, 1500);
    return () => clearTimeout(timer);
  }, [searchQuery, roomTypeTab]);
  ...
  const handleVote = (roomId: string, category: 'crowd' | 'spec', value: string) => {
    ...
    if (currentVote === value) {
      ...
      trackGAEvent('clear_vote', { room_id: roomId, category });
      ...
      // Cast vote
      trackGAEvent('cast_vote', { room_id: roomId, category, value });
  ...
  const handleSyncBRACUniversityLive = async (silent = false) => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error(`Server proxy responded with HTTP ${res.status}`);
      }
